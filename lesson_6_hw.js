...
function formatText(text) {
  const newText = text.trim().toLowerCase().split(' ').filter((item) => item !=='').join(' ')

  const newSentence = newText.replace(/(^|\.\s+)([a-z])/g, function (match, group1, group2) {
...
const newArray = arr.split('').filter((item) => /^[aeiouAEIOU]$/.test(item))
...
function clearStr(str) {
 const newString = str.replace(/[^\w\d\s]/g, '').replace(/\s+/g, ' ').trim()
 return newString
}
